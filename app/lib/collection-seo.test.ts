...
describe('collection crawl policy', () => {
  ...
  it('lets the filter policy win when pagination is also present', () => {
    expect(
      resolveCollectionSeoPolicy(
        'https://www.wandini.de/en/collections/wallpaper?f=%7B%22available%22%3Atrue%7D&direction=next&cursor=page-2',
      ),
    ).toEqual({
